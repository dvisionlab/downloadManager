 ...
 */
function concat(adding, actual, activeKey, ...args) {
    adding.forEach(item => {
    ...
    if (activeKey) {
        actual.sort((a, b) => {
            if (b.key == activeKey && a.key !== activeKey)
                return 1;
            if (a.key == activeKey && b.key !== activeKey)
                return -1;
            return 0;
        });
    }
 ...
 */
function alternate(adding, actual, ...args) {
    let newQueue = [];
 ...
 */
function propagate(adding, actual, activeKey, ...args) {
 ...
 */
function threeParted(adding, actual, activeKey, activeIndex, qs) {
...
/// utils ///
function applyActiveIndex(q2, activeIndex) {
...
function restoreOriginalOrder(arr) {
...
function findLastIndex(arr, testFn) {
...
function orderArrayFromIndex(arr, index) {
    if (index === undefined) {
...
exports.default = { concat, alternate, propagate, threeParted };
