import type { downloadQueueItem, addingQueueItem, removingQueueItem, seriesData } from './types'
...
export class DownloadManager {
  ...
  constructor(strategy: keyof typeof strategiesFns = 'concat', verbose?: boolean) {
  ...
  get isDownloading() {
    return (
      Object.values(this.seriesData).reduce((acc, curr) => curr.isDownloading || acc, false) &&
   ...
   */
  addSeries(studyId: string, seriesId: string, imageIds: string[]) {
  ...
  reworkQueue() {
    ...
    this.downloadQueue = strategiesFns[this.strategy](this.addingQueue, this.downloadQueue)

    this.addingQueue = []
    this.removingQueue = []

    if (this.verbose) console.debug('downloadQueue', this.downloadQueue)
  ...
  getNextSlot(slotDimension: number) {
    ...
    if (this.verbose) console.debug('nextSlot', nextSlot)
    return nextSlot
  }
